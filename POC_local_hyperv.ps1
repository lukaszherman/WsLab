...
"@
        ...
        if ($VMConfig.Unattend -eq "NoDjoin" -or $VMConfig.SkipDjoin){
            ...
            $path="c:\$vmname.txt"
            Invoke-Command -VMGuid $DC.id -Credential $cred  -ScriptBlock {param($Name,$path,$Labconfig); djoin.exe /provision /domain $labconfig.DomainNetbiosName /machine $Name /savefile $path } -ArgumentList $Name,$path,$Labconfig
...
#region Set variables
    ...
    If (!$LabConfig.DomainNetbiosName){
        WriteErrorAndExit "`t DomainNetbiosName not detected. Exiting"
    }
...
#region Some Additional checks and prereqs configuration
    ...
    #Testing if lab already exists.
		...
		if ((Get-VM  -Name ($LabConfig.DC) -ErrorAction SilentlyContinue) -ne $null){
			$LABExists=$True
...
#region Finishing
    ...
    #list VMs 
        Get-VM  | Where-Object name -like "$($labconfig.Prefix)*"  | Where-Object {$_.Name -in $LABConfig.VMs.VMName} | ForEach-Object { WriteSuccess "Machine $($_.VMName) provisioned" }

    #configure HostResourceProtection on all VM CPUs
        WriteInfo "`t Configuring EnableHostResourceProtection on all VM processors"
        Get-VM  | Where-Object name -like "$($labconfig.Prefix)*"  | Where-Object {$_.Name -in $LABConfig.VMs.VMName} | Set-VMProcessor -EnableHostResourceProtection $true  -ErrorAction SilentlyContinue
    ...
    if ($labconfig.EnableGuestServiceInterface){
        ...
        Get-VM  -VMName "$($labconfig.Prefix)*" | Where-Object {$_.state -eq "Running" -or $_.state -eq "Off" -AND $_.Name -in $LABConfig.VMs.VMName} | Enable-VMIntegrationService -Name "Guest Service Interface"
        $TempVMs=Get-VM  -VMName "$($labconfig.Prefix)*" | Where-Object {$_.state -ne "Running" -and $_.state -ne "Off"  -AND $_.Name -in $LABConfig.VMs.VMName}
        if ($TempVMs){
    ...
    #Enable VMNics device naming
        ...
        Get-VM  -VMName "$($labconfig.Prefix)*" | Where-Object {$_.Generation -eq 2 -AND $_.Name -in $LABConfig.VMs.VMName} | Set-VMNetworkAdapter -DeviceNaming On
